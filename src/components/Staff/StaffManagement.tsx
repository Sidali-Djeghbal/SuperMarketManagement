...
export default function StaffManagement({
  ...
  const [formType, setFormType] = useState<"machine" | "worker" | "cashier">(
    "worker"
  );
      ...
      const employee: Employee =
        formType === "cashier"
          ? {
              ...
              staffType: "cashier",
              machineNumber: (newEmployee as Partial<Cashier>).machineNumber!,
            }
          : {
              id: Date.now(),
              name: newEmployee.name!,
  ...
  return (
    <div className="w-full h-full flex flex-col border border-[#595959]/45 p-6 rounded-lg shadow-lg bg-[#1A1A1A]">
              ...
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewMachine({
                      ...newMachine,
                      machineNumber: parseInt(e.target.value),
                    })
                  }
                />
              </div>
                ...
                {formType === "cashier" && (
                  <div className="flex flex-col gap-2">
                    ...
                    <input
                      ...
                      className="bg-[#2A2A2A] text-white p-3 rounded-md border border-[#3A3A3A] focus:border-blue-500 focus:outline-none transition-colors"
                      value={
                        (newEmployee as Partial<Cashier>).machineNumber || ""
                      }
                      onChange={(e) =>
                        setNewEmployee({
                          ...newEmployee,
      ...
      <div className="flex-1 overflow-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {machines.length === 0 ? (
              ...
              machines.map((machine) => (
                ...
                >
                  <div className="w-16 h-16 bg-purple-500/20 rounded-full flex items-center justify-center mb-8 text-2xl font-bold text-purple-400">
        ...
        <div>
          <h3 className="text-xl font-medium text-white mb-4">Employees</h3>
          <div className="grid grid-cols-5 p-4 bg-[#232323] text-sm mb-4 text-left gap-4 text-white font-medium rounded-t-lg">
            <div>Name</div>
            <div>Type</div>
            <div>Email</div>
            <div>Phone</div>
          ...
          {employees.length === 0 ? (
            <div className="text-center py-8 text-[#595959]">
              No employees found. Add your first employee using the buttons
              above.
            </div>
          ) : (
            employees.map((employee) => (
              <div
                key={employee.id}
                className="grid grid-cols-5 items-center text-white text-sm mb-2 p-4 hover:bg-[#232323]/50 rounded-md border border-transparent hover:border-[#3A3A3A] transition-all"
              >
                <div className="font-medium">{employee.name}</div>
                <div className="capitalize">
                  {employee.staffType}
                  {employee.staffType === "cashier" &&
                    ` (Machine ${(employee as Cashier).machineNumber})`}
                </div>
                <div>{employee.email}</div>
                <div>{employee.phone}</div>
                <div>
                  <button
                    onClick={() => handleRemoveItem(employee.id, "employee")}
